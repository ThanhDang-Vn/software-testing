...
| Test Cases (Req 3) | Generated 12 normal TCs + 3 edge case templates | Reviewed TC format and edge case accuracy | Device photo with student ID; ≥5 execution videos; fill Actual/Verdict |
...
**Structural limitation:** The AI treats all physical device test cases as stateless — it does not model thermal states, time-dependent behavior, or real-world misuse. This is why edge cases like the empty-pot dry-run and the mid-cook lid opening are beyond AI's spontaneous generation capability. Human testers with physical device experience are still essential for safety-critical test design on embedded hardware.
