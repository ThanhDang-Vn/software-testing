...
| | |
|---|---|
| **Objective** | Verify the temperature control buttons correctly increase/decrease the set temperature and the fryer maintains the target |
| **Input** | Empty basket; start from default temperature; adjust to 180°C then to 220°C |
| **Steps** | 1. Power on device<br>2. Note default temperature shown on display<br>3. Press **+** button repeatedly until display reads 220°C<br>4. Press **−** button repeatedly until display reads 180°C<br>5. Press **Start**; allow to run for 3 minutes<br>6. Press **Cancel** and note no error occurred during operation |
| **Expected** | Display updates in defined increments (e.g., 5°C steps); values do not exceed stated max (typically 200–230°C) or go below stated min (typically 80–100°C); device runs stably at set temp |
| **Actual** | |
| **Verdict** | |
...
| **Steps** | 1. Power on device<br>2. Set temperature to 180°C<br>3. Press **Time** button; set timer to **5 minutes** using +/− buttons<br>4. Press **Start**; note start time<br>5. Wait for auto-stop; note elapsed time<br>6. Verify device stopped at exactly 5 minutes |
...
| **Expected** | Device heats up during preheat; fan runs; no error; outer body warm to touch but safe; preheat completes within set time; ready indicator activates |
| **Actual** | |
| **Verdict** | |
...
| | |
|---|---|
| **Objective** | Verify Reheat mode (or low-temp cycle) warms leftover food to safe serving temperature without overcooking |
...
| **Steps** | 1. Power on device with empty basket<br>2. Press **+** button until temperature can no longer increase (max reached)<br>3. Note the maximum temperature displayed<br>4. Set time to 3 minutes; press **Start**<br>5. Run for full 3 minutes; observe for error codes, unusual smell, or shutdown<br>6. Press Cancel; wait for cool-down |
| **Expected** | Temperature does not exceed stated max (typically 200–230°C depending on model); no error codes; device runs stably; no burning smell from components; auto-stops at 0:00 |
| **Actual** | |
| **Verdict** | |
...
| **Expected** | Shake reminder alert (beep + display message) fires at approximately the midpoint (~8 min); device may auto-pause or continue cooking; alert clears after basket pull-and-reinsert or button press |
| **Actual** | |
| **Verdict** | |
...
| **Expected** | Second cycle starts normally without requiring cool-down; device does not trigger thermal overload or error; 2nd batch cooks to same quality as 1st; no error codes; no unusual sounds or smells |
| **Actual** | |
| **Verdict** | |
...
| **Expected** | Either (a) device starts and stops immediately (0 min = cook for 0 seconds → instant stop), OR (b) device rejects 0:00 and snaps to minimum allowed time (e.g., 1 min). Any defined, non-crashing behavior is PASS. Device freezing, displaying garbage, running indefinitely, or rebooting is FAIL |
| **Actual** | |
| **Verdict** | |

> **Why AI missed this:** AI-generated test cases for physical appliances assume the timer always starts from a positive minimum value (e.g., 1 or 5 minutes) and never reaches zero before the Start button. Applying BVA to a hardware timer control — specifically testing the exact 0:00 boundary — requires deliberate testing methodology that AI does not spontaneously apply to embedded device UI without explicit prompting.
...
| **Expected** | Device detects missing basket and shows an error code / warning beep, OR refuses to start. If device lacks basket sensor (basic models): heating element activates without airflow — device should trigger thermal cutoff within 1–2 min. Running indefinitely without shutdown is a FAIL (fire/burn hazard) |
| **Actual** | |
| **Verdict** | |
